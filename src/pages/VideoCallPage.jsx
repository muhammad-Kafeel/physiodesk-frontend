...
import {
  Video, Phone, ArrowLeft, Clock, Calendar,
...
const getCallStatus = (apptDate, apptTime) => {
  ...
  const openFrom    = new Date(apptStart.getTime() - 15 * 60 * 1000);
  const now         = new Date();
...
export default function VideoCallPage() {
  ...
  useEffect(() => {
    appointmentAPI.getById(appointmentId)
      .then(r => {
        ...
        if (!appt.video_room_id) { toast.error('No video room yet. Doctor must confirm first.'); navigate(-1); return; }
        if (appt.type !== 'video') { toast.error('This is an in-person appointment.'); navigate(-1); return; }
        // F11 — Payment gate: patient must pay before joining
        if (!appt.is_paid && user?.role === 'patient') {
          toast.error('Please complete payment before joining the video call.');
          navigate(`/payment/appointment/${appt.id}`);
          return;
        }
        setAppointment(appt);
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setCountdown(prev => { if (prev <= 1) { clearInterval(t); return null; } return prev - 1; });
    }, 60000);
  ...
  const startCall = () => {
    ...
    const { status, minutesUntil } = getCallStatus(appointment.appointment_date, appointment.appointment_time);
    if (status === 'EARLY') {
    ...
    if (status === 'LATE') { toast.error('This appointment time has passed. Please reschedule.'); return; }
    if (window.JitsiMeetExternalAPI) { setCallStarted(true); setTimeout(() => initJitsi(), 100); return; }
  ...
  const initJitsi = () => {
    ...
    jitsiApi.current = new window.JitsiMeetExternalAPI('meet.jit.si', {
      roomName, width: '100%', height: '100%', parentNode: jitsiContainer.current,
      userInfo: { displayName: isDoctor ? `Dr. ${user?.name}` : user?.name, email: user?.email },
      configOverwrite: { startWithAudioMuted: false, startWithVideoMuted: false, enableWelcomePage: false, disableDeepLinking: true, prejoinPageEnabled: true },
      interfaceConfigOverwrite: {
        SHOW_JITSI_WATERMARK: false, SHOW_WATERMARK_FOR_GUESTS: false, APP_NAME: 'PhysioDesk',
        TOOLBAR_BUTTONS: ['microphone','camera','desktop','fullscreen','hangup','chat','raisehand','tileview','videoquality'],
      },
    });
    jitsiApi.current.addEventListener('videoConferenceLeft', () => {
      jitsiApi.current = null; setCallEnded(true); setCallStarted(false);
    });
  };
  ...
  const endCall = () => {
    ...
    setCallEnded(true); setCallStarted(false);
  };
  ...
  if (loading) return (
    <Layout>
      <div style={{ textAlign:'center', paddingTop:80 }}>
        ...
        <p style={{ color:'var(--gray-400)', fontSize:14, marginTop:12 }}>Loading consultation room...</p>
      </div>
    </Layout>
  );
  ...
  const isDoctor = user?.role === 'doctor';
  const { status: callStatus } = getCallStatus(appointment.appointment_date, appointment.appointment_time);
      ...
      <div className="vc-wrap">
        <div className="vc-topbar">
          ...
          <div className="vc-title-wrap">
            ...
            <span className="vc-title">{callStarted ? 'Live Consultation' : 'Video Consultation'}</span>
          </div>
        ...
        <div className="vc-body">
              ...
              <div className="vc-lobby-card">
                ...
                {callStatus === 'EARLY' && (
                  <div className="vc-early-box">
                    ...
                    <p className="vc-early-time">{fmtTime(appointment.appointment_time)} · {fmtDate(appointment.appointment_date)}</p>
                    <p className="vc-early-sub">
                      You can join <strong>15 minutes before</strong> the scheduled time.
                      {countdown !== null && countdown > 0 && <span className="vc-countdown"> Opens in {countdown} min</span>}
                    </p>
                  </div>
                )}
                ...
                {callStatus === 'OPEN' && (
                  <p className="vc-lobby-sub">You'll be connected to a secure, encrypted video room. Allow camera &amp; microphone when your browser asks.</p>
                )}

                <div className="vc-room-info">
                  <div className="vc-room-row"><span>Date</span><span>{fmtDate(appointment.appointment_date)}</span></div>
                  <div className="vc-room-row"><span>Time</span><span>{fmtTime(appointment.appointment_time)}</span></div>
                  <div className="vc-room-row"><span>Window</span><span>15 min before → 90 min after</span></div>
                  <div className="vc-room-row"><span>Platform</span><span>Jitsi Meet (Encrypted)</span></div>
                </div>
                ...
                {callStatus === 'OPEN' && (
                  <div className="vc-tips">
                    ...
                    <ul>
                      <li><CheckCircle size={12} color="var(--success)"/> Use Chrome or Firefox for best experience</li>
                      <li><CheckCircle size={12} color="var(--success)"/> Allow camera &amp; microphone when prompted</li>
                      <li><CheckCircle size={12} color="var(--success)"/> Be in a quiet, well-lit area</li>
                      <li><CheckCircle size={12} color="var(--success)"/> Stable internet connection required</li>
                    </ul>
                  </div>
                )}
          ...
          {callEnded && (
            <div className="vc-lobby">
              <div className="vc-lobby-card">
                <div className="vc-ended-icon"><CheckCircle size={56} color="var(--success)"/></div>
