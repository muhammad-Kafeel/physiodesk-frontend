...
import { Video, Phone, ArrowLeft, User, Clock, Calendar } from 'lucide-react';
...
export default function VideoCallPage() {
    ...
    appointmentAPI.getById(appointmentId)
      .then(r => {
        ...
        if (!appt.video_room_id) {
          toast.error('This appointment does not have a video room yet. Doctor must confirm first.');
          navigate(-1);
          return;
        }
        if (appt.type !== 'video') {
  ...
  const initJitsi = () => {
    ...
    const roomName = appointment.video_room_id.replace(/[^a-zA-Z0-9-]/g, '-');
    ...
    jitsiApi.current = new window.JitsiMeetExternalAPI('meet.jit.si', {
      ...
      interfaceConfigOverwrite: {
        TOOLBAR_BUTTONS: [
          ...
          'chat', 'recording', 'raisehand', 'videoquality',
          'filmstrip', 'invite', 'tileview', 'select-background',
        ],
    ...
    <Layout noFooter>
      <div className="vc-wrap">
        ...
        <div className="vc-body">
              ...
              <div className="vc-lobby-card">
                ...
                <div className="vc-participants">
                  <div className="vc-participant">
                    <div className="vc-avatar vc-avatar-doc">
                      {doc.name?.[0] || 'D'}
                    </div>
                    <p className="vc-part-name">Dr. {doc.name}</p>
                    <p className="vc-part-role">Doctor</p>
                  </div>
                  <div className="vc-call-icon">
                    <Video size={28} color="white"/>
                  </div>
                  <div className="vc-participant">
                    <div className="vc-avatar vc-avatar-pat">
                      {pat.name?.[0] || 'P'}
                    </div>
                ...
                <p className="vc-lobby-sub">
                  You'll be connected to a secure video room. Make sure your
                  camera and microphone are allowed in your browser.
                </p>
            ...
            <div className="vc-call-container">
              ...
              <div className="vc-call-bar">
                <div className="vc-call-info">
                  ...
                  <span>Live with Dr. {doc.name}</span>
                </div>
          ...
          {callEnded && (
            <div className="vc-lobby">
              <div className="vc-lobby-card vc-ended-card">
                <div className="vc-ended-icon">📋</div>
                <h2 className="vc-lobby-title">Consultation Ended</h2>
                <p className="vc-lobby-sub">
                  Your video consultation has ended. {isDoctor
                ...
                <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
                  {isDoctor && (
                    <Link to={`/doctor/write-prescription/${appointmentId}`}
                  ...
                  <button className="btn-outline-pd" onClick={() => { setCallEnded(false); }}>
                    🔄 Rejoin Call
                  </button>
                </div>
              </div>
